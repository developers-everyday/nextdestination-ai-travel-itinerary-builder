...
const HotelDetailsPanel: React.FC<HotelDetailsPanelProps> = ({ onBack, onSelect, searchData }) => {
    ...
    const performSearch = useCallback((query?: string, location?: google.maps.LatLngLiteral) => {
        ...
        const request: google.maps.places.TextSearchRequest | google.maps.places.PlaceSearchRequest = query
            ? { query }
            : {
                location: location as unknown as google.maps.LatLng,
                radius: 5000,
                type: 'lodging',
                keyword: 'hotel'
            };
        ...
        const callback = (results: google.maps.places.PlaceResult[] | null, status: google.maps.places.PlacesServiceStatus) => {
            if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
                ...
                } else if (results.length > 0 && results[0].geometry?.location) {
                    setLastSearchCenter({
    ...
    const handleCameraChanged = (ev: any) => {
        ...
        if (lastSearchCenter) {
            ...
            if (dist > 0.02) setShowSearchAreaButton(true);
        }
    };

    const handleSearchArea = () => {
        if (currentMapCenter) performSearch(undefined, currentMapCenter);
    };
                ...
                <div className="flex-1 bg-[#e2e8f0] relative overflow-hidden">
                    ...
                    >
                        {hotels.map((hotel) => (
                            ...
                            >
                                <div
                                    className={`group relative flex flex-col items-center cursor-pointer transition-all duration-300 ${(hoveredHotelId === hotel.id || selectedHotel?.id === hotel.id) ? 'scale-110 z-50' : 'hover:scale-105'}`}
                                    onMouseEnter={() => setHoveredHotelId(hotel.id)}
                                >
                                    <div className={`p-2 rounded-full border-2 border-white shadow-lg transition-colors ${(hoveredHotelId === hotel.id || selectedHotel?.id === hotel.id) ? 'bg-indigo-600' : 'bg-white'}`}>
                                        <MapPin className={`w-4 h-4 ${(hoveredHotelId === hotel.id || selectedHotel?.id === hotel.id) ? 'text-white' : 'text-slate-700'}`} />
                                    </div>
                                </div>
                            </AdvancedMarker>
                        ))}
                    ...
                    {showSearchAreaButton && (
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10">
                            <button
                                ...
                                className="bg-white px-4 py-2 rounded-full shadow-lg font-bold text-sm text-indigo-600 hover:bg-slate-50 transition-all flex items-center gap-2"
                            >
