...
import { Itinerary, ItineraryItem, saveItinerary, saveItineraryToBackend, updateItineraryPrivacy } from '@nextdestination/shared';
...
const SortableActivityItem = ({ id, item, index, dayIndex, onRemove, onUpdate, searchBounds, onFocusMap }: SortableItemProps & { onUpdate: (dayIndex: number, activityIndex: number, data: any) => void }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  React.useEffect(() => {
    ...
    if (searchValue.length > 2) {
      const timer = setTimeout(async () => {
        try {
          const request: google.maps.places.AutocompleteRequest = { input: searchValue };
  ...
  const handleSelectSuggestion = async (suggestion: google.maps.places.AutocompleteSuggestion) => {
    ...
    if (!editTitle) setEditTitle(placeText);

    const updates: any = { description: newDesc, activity: editTitle || placeText };
    ...
    if (placeId && window.google) {
      try {
        ...
        if (place.location) {
          const coords: [number, number] = [place.location.lng(), place.location.lat()];
  ...
  return (
    <div ref={setNodeRef} style={style} className="mb-4">
      <div
        onClick={() => {
          if (!isEditing && item.coordinates) onFocusMap(item.coordinates);
        }}
      ...
      >
        {!isEditing && (
          <button
            onClick={(e) => { e.stopPropagation(); onRemove(dayIndex, index); }}
        ...
        <div className={`flex items-center justify-between ${isEditing ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                ...
                <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-slate-200 rounded-lg shadow-xl z-[60] overflow-hidden">
                  {suggestions.map((suggestion) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-slate-400 truncate">{suggestion.placePrediction?.secondaryText?.text}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex gap-2">
            {isEditing ? (
              ...
              <>
                <button onClick={(e) => { e.stopPropagation(); setIsEditing(true); }} className="py-1 px-3 rounded-lg border border-slate-200 font-semibold text-slate-600 text-[10px] hover:bg-slate-50 transition-all">Edit</button>
                <button
                  onClick={(e) => { e.stopPropagation(); item.coordinates && onFocusMap(item.coordinates); }}
...
const ItineraryBuilder: React.FC<Props> = ({
        ...
        try {
          ...
          if (places && places.length > 0) {
            ...
            if (place.viewport) {
              ...
              if (place.location) setFocusedLocation([place.location.lng(), place.location.lat()]);
  ...
  const isOwner = user && (data.userId === user.id || !data.userId);
  ...
  const handleSaveTrip = React.useCallback(async () => {
    ...
    try {
      const itineraryToSave = isOwner ? data : { ...data, id: undefined, userId: user.id };
      const saved = await saveItinerary(itineraryToSave, `Trip to ${data.destination}`);

      if (onItineraryChange && (!data.id || !isOwner)) onItineraryChange(saved);
  ...
  }, [user, data, session, isOwner, router, onItineraryChange]);
  ...
  useEffect(() => {
    const handleVoiceHotelSearch = (e: Event) => {
      ...
      if (location) { setSearchData({ location }); setRightPanelMode('HOTEL_DETAILS'); }
    };
  ...
  const handleTogglePrivacy = async () => {
    ...
    if (onItineraryChange) onItineraryChange({ ...data, isPublic: newPrivacyStatus });
    try {
      if (session?.access_token) await updateItineraryPrivacy(data.id, newPrivacyStatus, session.access_token);
    } catch (err) {
  ...
  const handleShare = async () => {
    ...
    try {
      const id = await saveItineraryToBackend(data, session?.access_token, true);
  ...
  const handleSelectFlight = (flight: any) => {
    const context = searchData?.context || 'activity';
    const newItem = {
      type: 'flight' as 'flight',
    ...
    if (context === 'arrival') onAddActivity(0, newItem);
    else if (context === 'departure') onAddActivity(totalDays - 1, newItem);
    else onAddActivity(safeDayIndex, newItem);
  ...
  const handleAddActivityFromPanel = (activity: any) => {
    const newItem = {
      activity: activity.name || activity.activity || activity.text,
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) onReorderActivity(safeDayIndex, oldIndex, newIndex);
    }
  };

  React.useEffect(() => {
    if (activeDay > totalDays && totalDays > 0) setActiveDay(totalDays);
  }, [totalDays, activeDay]);
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                onChange={(e) => { if (onItineraryChange) onItineraryChange({ ...data, startDate: e.target.value }); }}
          ...
          <div className="flex items-center gap-3 relative" ref={node => {
            if (node) {
              const handleClickOutside = (e: MouseEvent) => {
                if (!node.contains(e.target as Node)) setIsProfileOpen(false);
              };
            ...
            <button
              ...
              className={`flex items-center gap-2 px-2 sm:px-4 py-2 rounded-full font-bold text-sm transition-all
                ${shareStatus === 'shared'
            ...
            {isOwner && data.id && (
              <button
                ...
                className={`flex items-center gap-2 px-2 sm:px-4 py-2 rounded-full font-bold text-sm transition-all border
                  ${data.isPublic
              ...
              <div className="flex-1 overflow-y-auto px-6 py-2 space-y-4 scrollbar-hide pb-24 md:pb-20">
                ...
                {activeDay === 1 && data.hasArrivalFlight !== false && (
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 relative group mb-4 hover:shadow-md transition-shadow">
                    <button onClick={onRemoveArrivalFlight} className="absolute top-3 right-3 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1 z-10">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
                ...
                {currentDay.hasHotel !== false && !currentDay.activities.some(a => a.type === 'hotel') && (
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 relative group mb-4 hover:shadow-md transition-shadow">
                    <button onClick={() => onRemoveHotel(safeDayIndex)} className="absolute top-3 right-3 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1 z-10">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
                ...
                {activeDay === totalDays && data.hasDepartureFlight !== false && (
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 relative group mt-8 hover:shadow-md transition-shadow">
                    <button onClick={onRemoveDepartureFlight} className="absolute top-3 right-3 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1 z-10">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
                  ...
                  <div className="flex-1 relative">
                    ...
                    {isAddMenuOpen && (
                      <div className="absolute bottom-full left-0 mb-2 w-full bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-50 animate-fade-in-up">
                        <button
                          onClick={() => { setIsAddMenuOpen(false); onAddActivity(safeDayIndex, {}); }}
