                ...
                <div className="flex items-center gap-8 overflow-x-auto scrollbar-hide pb-2">
                    {categories.map((category) => (
                        <button
                            ...
                            className={`flex flex-col items-center gap-2 min-w-max group transition-all duration-200 ${selectedCategory === category.id
                                    ? 'text-slate-900 opacity-100'
                                    : 'text-slate-500 opacity-70 hover:opacity-100'
                                }`}
                        >
                            <div className={`text-2xl transition-transform duration-200 ${selectedCategory === category.id ? 'scale-110' : 'group-hover:scale-110'
                                }`}>
                                {category.id === 'city' ? (
                                    // Custom SVG for city since emoji might vary
                            ...
                            <span className={`text-xs font-bold whitespace-nowrap pb-2 border-b-2 transition-all duration-200 ${selectedCategory === category.id
                                    ? 'border-slate-900'
